...
def cmd_record(cfg: Config, args) -> int:
    ...
    if args.no_compile:
        ...
        return 0
    return _compile_and_report(cfg, sdir, args.stt,
                               open_after=cfg.open_after and not args.no_open)


def cmd_compile(cfg: Config, args) -> int:
    return _compile_and_report(cfg, _resolve_session(cfg, args.session), args.stt, open_after=False)
...
def _compile_and_report(cfg: Config, sdir: Path, stt: str | None, open_after: bool) -> int:
    from .compiler.compile import compile_session
    from .compiler.render import fmt_clock

    try:
        result = compile_session(sdir, cfg, stt=stt)
    except SystemExit as exc:
        print(f"compile failed: {exc}", file=sys.stderr)
        print(f"the recording is safe — retry with: sj compile {sdir}", file=sys.stderr)
        return 1
    print(f"{result.words} words · {result.segments} segments · {len(result.anchors)} figure(s)")
    for n, anchor in enumerate(result.anchors, start=1):
        via = " (click)" if anchor.clicked else ""
        print(f"  fig-{n:02d} ← “{anchor.phrase}” at {fmt_clock(anchor.t)}{via}")
